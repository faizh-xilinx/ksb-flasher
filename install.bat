...
:: ── Detect mode ────────────────────────────────────────────────
set "MODE=install"
if /i "%~1"=="--fresh" goto :do_install
if /i "%~1"=="--clean" goto :do_install
...
if exist "%INSTALL_DIR%\_python\python.exe" (
    ...
    powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%PYTHON_URL%' -OutFile '%TEMP%\python-embed.zip' -UseBasicParsing"
    ...
    echo       Extracting...
    powershell -NoProfile -Command "Expand-Archive -Path '%TEMP%\python-embed.zip' -DestinationPath '%INSTALL_DIR%\_python' -Force"
...
if not exist "%INSTALL_DIR%\_python\Lib\site-packages\pip" (
    powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%PIP_URL%' -OutFile '%TEMP%\get-pip.py' -UseBasicParsing"
...
if not exist "%INSTALL_DIR%\_python\Lib\site-packages\aiohttp" (
...
call :build_exe
if not exist "%INSTALL_DIR%\dist\KSB_Flasher.exe" (
...
"%INSTALL_DIR%\_python\python.exe" -m pip install aiohttp asyncssh pyinstaller --no-warn-script-location --quiet --upgrade >nul 2>&1

echo [2/3] Updating application files...
call :copy_files

echo [3/3] Rebuilding KSB_Flasher.exe...
call :build_exe
if not exist "%INSTALL_DIR%\dist\KSB_Flasher.exe" (
...
copy /y "%SCRIPT_DIR%ksb_hosts.csv" "%INSTALL_DIR%\ksb_hosts.csv" >nul 2>&1
